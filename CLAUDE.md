...
- **Perfil** (`core/Profile.js`): plantilla de Pokémon (fichas de `core/PokemonSnapshot.js` con `uid`), formación (`teamUids`: el primero es el líder; protagonista y compañero siempre dentro), banco, almacén, rango, día, mazmorras completadas, misiones y escenas de la historia vistas. La mochila y la cartera en juego son `game.inventory` y `game.coins`.
...
- `data/`: JSON de Pokémon, movimientos, objetos, tipos, evoluciones, pisos, mazmorras, pueblo, test de personalidad y consejos de carga (`tips.json`: salen en el registro cada dos pisos; `core/Tips.js` los rota con los turnos jugados, sin tocar el RNG). Si cambia una mecánica, revisa sus consejos; `tests/unit/tips.test.js` vigila que no vuelvan las capturas ni la muerte permanente.
