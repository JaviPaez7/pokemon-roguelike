...
- **Mazmorras** (`core/Dungeons.js`, `data/dungeons.json`): cada una es un tramo de los 50 pisos globales de `floors.json`. `game._currentFloor` es el piso global (decide zona, enemigos, jefe y dificultad) y `game.getCurrentFloor()` el que ve el jugador. La Torre del Desafío recorre los 50 con reglas roguelike: copias de nivel 5, kit fijo y lo de fuera guardado en `profile.stash`.
  - `unlock` en `dungeons.json` admite `{ cleared: "id" }`, `{ cleared: ["a", "b"] }` (todas) o `{ story: "F-3" }` (haber visto esa escena). `isUnlocked(dungeon, cleared, seen)`; con un perfil, `profileDungeons(profile)`.
  - En `floors.json`, una zona puede traer `weather` (probabilidad y climas posibles) y su jefe, `moves` (su kit, en vez de su lista de movimientos) y `hpMultiplier`.
...
- **Posjuego** («Las leyendas del valle», H5): cuatro mazmorras `postgame` en los pisos globales 51-84. Cumbre Escarcha (Articuno), Pico Tronador (Zapdos) y Caldera Ascua (Moltres) se abren a la vez al ver F-3 (el final) y se hacen en cualquier orden; el Jardín del Primer Sueño (Mew), al completar las tres. Las escenas son las L del guion (capítulo 8): L-0 presenta los picos tras los créditos (o, en partidas que ya habían acabado, en cuanto pasa algo en el pueblo) y L4-A trae el sobre del jardín. El tablón también da encargos allí (dificultad ★).
  - Los legendarios de `legendBosses` (`recruitment.json`) no pasan por la tirada: al derrotarlos como jefe se ofrecen siempre a unirse, hasta que se unen (como mucho uno de cada especie; `legendJoins` en `core/Recruitment.js`, `offerLegendRecruit` en `systems/RecruitSystem.js`). Se crean de nuevo a su nivel, sin los PS de jefe y con su kit. Mewtwo no está en la lista.
...
- Los menús eligen opción con el ratón al moverlo (`mousemove`), no con `mouseover`: al desplazarse una lista larga con el teclado bajo un ratón quieto, el navegador lanza `mouseover` y la selección saltaba.
