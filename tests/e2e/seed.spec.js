import { test, expect, startNewGame, playerPosition } from './fixtures.js';
...
test('la misma semilla en la URL genera la misma mazmorra', async ({ page }) => {
  // startNewGame recarga la página: cada vuelta es un juego desde cero
  const prints = [];
  for (let i = 0; i < 2; i++) {
    await startNewGame(page, { seed: 777 });
...
test('semillas distintas generan mazmorras distintas', async ({ page }) => {
  await startNewGame(page, { seed: 1 });
  const first = await floorFingerprint(page);
  await startNewGame(page, { seed: 2 });
